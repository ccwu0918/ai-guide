# TRAE SOLO：AI 主導的全棧開發工具

你好，我是魚皮。

在前面的文章中，我們學習了各種 AI 程式設計工具。無論是 Cursor 還是 Claude Code，都是 **人類主導 + AI 輔助** 的模式，你需要不斷地和 AI 對話、確認、修改。

但如果你想讓 AI 更主動一些，自己規劃任務、自動執行開發，該用什麼工具呢？

這篇文章，我會介紹 TRAE SOLO，一個讓 AI 主導開發任務的全棧開發工具。



## 一、什麼是 TRAE SOLO？

[TRAE](https://www.trae.ai/) 是位元組跳動推出的 AI 原生程式設計工具，有 2 種開發模式：
...
打個比方，用 Cursor 就像你是專案經理，AI 是程式設計師，你需要不斷地給 AI 分配任務、檢查成果、提出修改意見。而用 TRAE SOLO，AI 就是專案經理 + 程式設計師，你只需要告訴它目標，它會自己規劃、自己開發、自己測試。

根據 2026 年的最新評測，TRAE 在 AI 程式設計軟體中表現優異，特別是它的可自定義智慧體系統，允許使用者像組建團隊一樣，根據專案需求定義和配置不同角色、技能的 AI 智慧體。
...
TRAE SOLO 支援多工並行執行，可以同時開發前端和後端，大大提高效率。
...
⭐️ 推薦直接看魚皮的影片教程，講解更詳細：https://www.bilibili.com/video/BV1yMn3zuE7L
...
準備好工具後，進入 TRAE 的 SOLO 模式，開啟一個空白的專案資料夾。
...
可以點選檢視某個原型具體的設計，只需要選中你看上的原型圖，點選新增到對話，TRAE 就會自動把原型圖關聯到發給 AI 的對話中。
...
把使用者資料儲存在哪裡？怎麼讓程式對接 AI 大模型？這都是我們要解決的問題。
...
[Supabase](https://supabase.com/) 是開源的後端即服務（BaaS）平臺，提供資料庫儲存、使用者身份驗證、即時 API 等功能。
...
等待期間，你可以到 [程式設計導航](https://codefather.cn/) 上看看免費的程式設計學習路線。TRAE 自帶訊息通知功能，AI 完成任務後會提醒我們。
...
最終，你看到的這個小程式，就是我最佳化後的成品，效果還不錯吧。
...
首先是 **AI 主導開發**，你不需要不斷地和 AI 對話，AI 會自己規劃任務、自動執行。而且 **服務整合能力強**，可以傻瓜式接入 Supabase、Stripe 等服務，不用看文件。

還有就是 **文件自動生成**，符合企業規範的開發流程，生成的文件很專業。而且 **國內版訪問速度快**，對中文使用者友好。

當然也有一些侷限。

首先是 **需要一定的引導**，雖然 AI 能自主執行，但你的需求描述越清晰，AI 的執行效果就越好。而且 **生成的程式碼可能有 Bug**，需要人工測試和修復。

另外，**上下文管理很重要**。如果對話太長，AI 可能會記憶混亂，需要適時新開對話。

價格上，TRAE 有免費版和付費版。免費版有使用限制，付費版根據使用量計費。



## 五、TRAE SOLO 實戰建議

如果你想嘗試 TRAE SOLO，我有幾個建議：

1、明確需求

雖然 AI 能自主規劃，但你的需求越明確，AI 的執行效果就越好。建議：
...
不要一次性讓 AI 生成整個專案，而是分步驟：

- 先生成後端，測試透過
- 再生成前端，測試透過
- 最後最佳化樣式和細節

每完成一步，都提交一次程式碼，這樣出問題也能回退。



3、及時修復 Bug

AI 生成的程式碼可能有 Bug，這很正常。發現問題就立刻讓 AI 修復，不要等到最後問題越堆越多。

把報錯資訊完整地發給 AI，它一般都能修復。
...
看到這裡，相信你已經對 TRAE SOLO 有了全面的瞭解。

**TRAE SOLO 是 AI 程式設計工具的一次重要進化**，從 “將 AI 整合到工具中”，進化為 “將開發工具整合到 AI 中”。

![](https://pic.yupi.icu/1/image-20250928220158367.png)

感覺開發工具只是 AI 的一個玩物，它可以自由地操作編輯器、終端、瀏覽器、文件、整合服務等等來自主完成任務，確實比以前更高效智慧了，也能完成包含後端的完整專案。

但是開發過程中大家要注意，現在 **AI 很喜歡把簡單的事情搞複雜**，所以最好人工仔細確認需求文件和方案文件，並且像搭積木一樣，一步一步地完成開發。

快試試用 TRAE SOLO 做一個小專案，體驗一下 AI 主導開發的感覺吧~ 🛫
