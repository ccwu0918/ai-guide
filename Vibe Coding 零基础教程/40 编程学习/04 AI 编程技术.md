...
作為程式設計師，咱們不光要會用 AI 工具、能利用 AI 開發專案，還要能夠自主開發 AI 專案，把 AI 的能力接入到自己的專案中。
...
[Spring AI](https://docs.spring.io/spring-ai/reference/getting-started.html) 是 Spring 官方推出的 AI 開發框架，經過 2 年的沉澱，在 2025 年 5 月正式釋出了 1.0 版本。

![Spring AI 1.0 釋出](https://pic.yupi.icu/1/1747881171718-91ac3eb5-049b-4510-8012-6736c40c9c95.png)

Spring AI 的優勢在於 **更容易和主流 Java 開發框架 Spring 整合**，上手難度較低。它提供了很多現成的方法來幫我們提高開發 AI 應用的效率，比如快速對接大模型、儲存會話上下文、對接向量資料庫實現 RAG 等等。
...
- RAG 檢索增強生成：完整的 RAG 流程支援，包括文件載入、向量儲存、檢索最佳化
- 工具呼叫：透過註解快速定義工具，讓 AI 呼叫外部服務
...
[Spring AI Alibaba](https://java2ai.com/) 是阿里巴巴基於 Spring AI 推出的國內版本，專門針對國內的 AI 生態做了最佳化。
...
- AI Service：宣告式開發，透過註解快速構建 AI 服務
- 會話記憶：支援多種會話記憶策略和持久化
...
先透過文字嵌入模型，將企業各種文件轉化為向量，存入向量資料庫；當使用者提問時，系統在向量資料庫檢索相關向量資料，找到最相似文件片段，和問題一起輸入大模型處理。這樣一來，大模型能夠基於企業真實資料作答，更準確貼合實際。
...
目前主流的 RAG 方案已經演化出十幾種變體，按照複雜度可以分為幾個層次：

- 基礎方案：Naive RAG（標準的切塊→向量檢索→生成流程）、Multi-Query RAG（多種問法分別檢索再合併）
- 檢索最佳化：Hybrid Search（混合向量搜尋和關鍵詞搜尋）、Reranking（精排模型過濾噪聲）、語義分塊（按語義邊界切分文件）
- 反思糾錯：Corrective RAG（檢索後質檢，搜不到就回退 Web 搜尋）、Self-RAG（生成時自我審視是否有文件支撐）
- 結構化增強：GraphRAG（知識圖譜支援跨文件推理）、Text-to-SQL RAG（自然語言翻譯成 SQL 查資料庫）
- 智慧體驅動：Agentic RAG（AI Agent 自主決策檢索策略）、Multi-Agent RAG（多個專職 Agent 協作）

![](https://pic.yupi.icu/1/1776651602499-afc9fc4b-6105-4b96-a3cd-a6681bf69480-20260430225809480.png)

對於初學者，建議從 Naive RAG 開始，發現哪個環節不夠好，再針對性引入進階方案。生產環境推薦 Hybrid Search + Reranking 作為基礎配置。如果資料來源多且複雜，可以考慮 Agentic RAG。

![](https://pic.yupi.icu/1/1776652755436-d1f52895-b98a-4e33-8c86-6ce0322ace05-20260430225845924.png)

主流的 RAG 開發工具和框架包括 LangChain / LangGraph、LlamaIndex、以及一體化平臺 Dify 和 RAGFlow 等，感興趣的同學可以進一步瞭解。
...
比如做個智慧導購系統，顧客既可以輸入文字描述想要的商品，系統也能識別顧客上傳的商品圖片，甚至可以理解顧客透過語音提出的購物需求。

![多模態](https://pic.yupi.icu/1/1743563981663-8c9f4746-03dc-4b32-8477-ba9a9042922c.png)

想開發多模態應用，咱們要學習模態轉換技術，比如文字轉語音（TTS）、語音轉文字（STT）、光學字元識別（OCR）等。不過這些都有現成的工具庫或者雲服務，掌握呼叫方法就行。
...
就拿開發影片網站為例，使用者說了 “幫我開發一個 Dilidili 影片網站並部署上線” 的指令時，智慧體首先會深入理解任務內容，透過推理梳理出一系列執行步驟，包括明確需求、設計方案、搭建框架、生成程式碼、部署上線等。接下來，智慧體就會呼叫相應的工具來執行這些行動。
...
- Apache Tika：功能強大的檔案解析器工具庫，支援解析 PDF、Word、Excel、PowerPoint 等各種文件
- Playwright：用於模擬瀏覽器行為的工具庫，AI 需要執行網頁、抓取網頁資料、自動化測試時都能派上用場
...
專案最終是要部署上線的嘛，所以我們還要掌握高效的部署工具。如果是個人學習使用、想快速上線自己的 AI 小應用，可以試試下面這些平臺：

- [Vercel](https://vercel.com/)：適合前端應用的部署平臺，支援自動構建、線上瀏覽、CDN 分發，而且還免費提供可訪問的域名
- [Sealos](https://sealos.io/)：雲原生應用管理平臺，支援 Kubernetes 叢集管理
- [Railway](https://railway.com/)：能讓開發人員輕鬆部署 Docker 容器，無需操心伺服器配置與運維
...
- AI 零程式碼應用生成平臺：LangChain4j AI 智慧體、LangGraph4j 工作流、微服務架構
- AI 答題應用平臺：React 跨端小程式、Vue3 AI 應用、分庫分表、SSE 實時推送

這些專案都是企業級的真實專案，做完後可以直接寫進簡歷。
...
- AI 應用生成平臺：https://github.com/liyupi/yu-ai-code-mother
...
在我免費開源的 [AI 知識庫](https://github.com/liyupi/ai-guide) 中，彙總收集了最新最全的 AI 知識，幫助大家更好地適應 AI 時代的到來。
...
我的答案仍然是 “會”。因為程式設計師本身就是需要持續學習和實踐來保持競爭力的，只要大家能夠學會我提到的這些知識，多關注 AI 的前沿資訊，相信 AI 不會搶走咱們程式設計師的飯碗，而是成為咱們改造世界的槓桿。
