# Vercel AI 閘道器 - AI 減壓小能手專案實戰

這篇文章的重點不在於開發，而在於學習 AI 閘道器的概念和使用方法。透過漫畫式的講解和 Vibe Coding 專案實戰，你可以瞭解如何透過 Vercel AI Gateway 在 AI 應用中靈活切換不同的模型，降低開發成本和維護難度。適合想學習 AI 閘道器技術、需要快速對接多個 AI 模型的同學。
...
你有些不爽：你這是站著說話不腰疼，這麼多要求，不得寫一大坨程式碼？

魚皮：老闆說的這些場景，都可以透過 **AI 閘道器** 解決呀~

你疑惑地問：閘道器？那是啥？

魚皮：閘道器就像是火車站的檢票口，所有乘客都要先經過檢票口，檢票員會檢查你的車票，然後指引你去正確的站臺。

![](https://pic.yupi.icu/1/1761645506547-26d820e6-cda3-482e-a7e9-dcadcbbb0445.png)

在系統架構中，前端使用者發來的請求要先經過閘道器，閘道器會統一處理使用者認證、攔截惡意請求、控制流量、監控統計請求等等，然後把請求轉發到後端伺服器進行處理。

![](https://pic.yupi.icu/1/1761645542774-a2da29dc-3d2c-4e1b-adcf-dc5dd59d0881.png)

你點了點頭：哇哦，這樣一來，如果我後端有多個服務，不用給每個服務單獨寫這些功能了。

魚皮：沒錯，而且如果一個後端服務掛了，閘道器還可以自動將請求轉發到其他服務。

![](https://pic.yupi.icu/1/1761645571947-8cb4a141-7d6f-4add-a82b-7a70716f23e7.png)

你有些好奇了：那你剛剛提到的 AI 閘道器又是什麼？

魚皮：傳統的 API 閘道器通常放在你的應用和各種後端服務之間；而 AI 閘道器是專門為 AI 應用設計的，放在你的應用和各種 AI 模型服務之間（比如 OpenAI、通義千問、DeepSeek 等等）。

![](https://pic.yupi.icu/1/1761645615469-787296d8-1fb6-4452-b406-c79ef537f193.png)

你的應用只需要傳送 **標準請求** 給 AI 閘道器，它就會自動幫你完成使用者鑑權、限流控制、安全防護、故障轉移、負載均衡、監控統計等一系列複雜的操作，並且將請求轉發給 AI 大模型進行處理。

![](https://pic.yupi.icu/1/1761645642401-683e786e-3e06-420a-abce-cd43f7bfa901.png)

如果你要對接不同的大模型，只需要修改標準請求中的大模型名稱，AI 閘道器就會幫你完成路由，不用針對每種模型寫一套對接的程式碼啦~

![](https://pic.yupi.icu/1/1761645657980-b463d0b2-eecd-4635-99ee-fed9f121bf4e.png)

你歡呼雀躍：這也太強了，有了 AI 閘道器，老闆提到的所有問題都能解決了！那現在有哪些 AI 閘道器產品可以用呢？
...
很多 AI 玩家第一個接觸到的 AI 閘道器可能是 [OpenRouter](https://openrouter.ai/)，它更像是一個 AI 模型的聚合平臺，支援透過統一的介面對接幾百種模型。
...
你問到：那還有專門面向開發者的 AI 閘道器產品嗎？

魚皮點點頭：當然，現在市面上已經有不少成熟的 AI 閘道器產品了。比如你上網一搜，排名靠前的是下面幾個：

1）[Vercel AI Gateway](https://vercel.com/ai-gateway)：這是最近很火的一個新產品，最大的特點是上手容易，而且 **零加價**，如果你使用自己的 API Key，閘道器本身不收額外費用。適合前端開發者快速搭建全棧 AI 應用。
...
3）[Kong AI Gateway](https://konghq.com/products/kong-ai-gateway)：Kong 本身就是一個很成熟的 API 閘道器，現在專門為 AI 場景做了增強，企業級功能比較完善。

![](https://pic.yupi.icu/1/image-20251018111819481.png)

4）[Higress AI](https://higress.ai/)：阿里雲開源的 AI 閘道器，支援 100 多種大模型的統一協議轉換，提供語義快取、令牌限流、MCP 轉換等企業級功能，適合有複雜 AI 整合需求的企業。

![](https://pic.yupi.icu/1/image-20251019163817976-20251028181254777.png)



你撓了撓頭：看起來也太複雜了吧，我應該怎麼上手 AI 閘道器呢？
...
首先開啟 [Vercel 官網](https://vercel.com/) 註冊賬號，繫結銀行卡可以免費獲得 5 美元的使用額度，對於學習測試來說夠用了。

![](https://pic.yupi.icu/1/1760687990497-90720fbb-0df6-4ede-87b8-64b8702994e9-20251028181254840.png)



然後在控制檯建立 API Key，注意不要洩露哦：
...
接下來，你可以透過官方提供的快速開始教程來建立專案並跑通 AI 對話的 Demo：
...
但是官方提供的 Demo 有點過於簡單了，我們不妨利用 AI 來做一個《減壓小能手》網站專案吧，讓使用者可以和專門幫人減壓的 AI 聊天。

這裡我選用的 AI 開發工具是 Cursor，直接讓 AI 幫我生成符合要求的完整前端 + 後端程式碼。

![](https://pic.yupi.icu/1/1761645829262-bd9950ef-6334-410f-ab27-140873cb56a4.png)

由於 Vercel AI Gateway 是比較新的玩意，AI 可能不理解它的用法，所以我直接把 Vercel AI Gateway 的官方文件扔給 Cursor，讓他透過文件來學習用法。
...
```markdown
你是一位專業的程式設計師，請幫我開發《減壓小能手》網站，使用者可以透過和專門幫人減壓的 AI 聊天來緩解壓力。

## 開發要求

1. 需要包含完整的前端和後端，後端使用 Node.js
2. 使用 Vercel 的 AI Gateway 實現 AI 能力，需要先透過官方文件來獲取儘可能多的用法：https://vercel.com/docs/ai-gateway/getting-started
3. 以完成核心功能為目標，確保專案可以正常執行，不用輸出文件、也不要做任何多餘的功能
4. 整體網站介面採用讓人放鬆的淺色，響應式適配各種尺寸的裝置
```



點選執行後，AI 會先呼叫 MCP 工具從網頁中獲取到資訊，這裡我用的是 `Firecrawl MCP`：

![](https://pic.yupi.icu/1/1760690503357-7412db47-4389-49e2-b42c-dc6eabdc283b-20251028181255024.png)



大概 6 分鐘左右，AI 完成了全部程式碼的生成。可惜這裡 AI 不是很聽話，還是給我生成了一堆文件，比生成程式碼本身的時間都多！

![](https://pic.yupi.icu/1/image-20251019161322770.png)



然後在根目錄下建立 `.env` 環境變數配置檔案，填入 AI Gateway 的 API Key：
...
你感嘆道：有了 AI + AI 閘道器這套組合拳，開發 AI 專案也太快了吧！

魚皮點點頭：沒錯，而且整個過程中我們不用擔心某個模型掛掉，閘道器會自動處理這些問題。
...
此外，還提供了 **可觀測性** 等企業級功能，幫你瞭解 AI 的呼叫情況和成本分析：
...
如果你的個人專案只需要簡單地呼叫單個 AI 模型，直接呼叫 API 就夠了；如果你個人或團隊的小專案需要用到 AI 閘道器的特性（比如整合多個模型），那麼可以選用 Vercel AI Gateway；而如果你要開發企業級應用，對安全性和穩定性要求很高，選用 Higress 或者 Kong 這種更專業的閘道器更合適，能幫你少寫億點點屎山程式碼 💩！
...
幾個月後，你用 AI 閘道器重構了公司的智慧客服系統，效果非常好。
